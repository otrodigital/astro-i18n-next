...
These are discovered when you specify `contentDirs` or `contentRoutes` in the config:
...
:::tip
Use `contentDirs` when you only need slug resolution helpers (e.g., for listing pages). Use `contentRoutes` when you also need the integration to handle route injection with per-locale translated slugs. See [Translated Routes](/guides/translated-routes/#content-collection-routes) for details.
:::
...
The `category` argument matches the key used in `contentDirs`, `contentRoutes`, or `slugMaps` (if provided manually).

:::note
Page slugs use the category `"pages"` internally. Content slugs use the key you defined in `contentDirs` or `contentRoutes`.
:::
