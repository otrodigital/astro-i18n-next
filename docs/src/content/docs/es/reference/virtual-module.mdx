...
declare module 'virtual:i18n' {
  ...
  export const getAllSlugPairs: (category: string, locale: string) => { canonical: string; localized: string }[];
...
Obtiene todos los pares de slugs para una categoría e idioma. Devuelve un array de objetos `{ canonical, localized }`.
